...
namespace Thread_.NET.BLL.Services
{
    public sealed class PostService : BaseService
    {
        ...
        {
            var post = (from t in _context.Posts where t.Id == postDto.Id && t.AuthorId == postDto.Author.Id select t).FirstOrDefault();
            var reaction = (from t in _context.PostReactions where t.PostId == post.Id select t);
            var comments = (from t in _context.Comments where t.PostId == post.Id select t);
