...
export class MainThreadComponent implements OnInit, OnDestroy {
    ...
    public deletePostHub: HubConnection;
    ...
    public SendDeletePost(post: Post) {
        const postSubscription = this.postService.deletePost(post);

        this.loading = true;

        postSubscription.pipe(takeUntil(this.unsubscribe$)).subscribe(
            (respPost) => {
                this.DeletePost(respPost.body);
                this.loading = false;
            },
            (error) => this.snackBarService.showErrorMessage(error)
        );
    }
    public DeletePost(deletepost: Post) { // исправить
        if (this.cachedPosts.some((x) => x.id === deletepost.id)) {
            this.cachedPosts = this.cachedPosts.filter((x) => x.id !== deletepost.id);
        }
        if (this.posts.some((x) => x.id === deletepost.id)) {
    ...
    public registerHub() {
        ...
        .withUrl(`${environment.apiUrl}/notifications/post`, {accessTokenFactory: () => accessToken }).build();
        ...
        this.deletePostHub = new HubConnectionBuilder().withUrl(`${environment.apiUrl}/notifications/post`,
        { accessTokenFactory: () => accessToken }).build();
        this.deletePostHub.start().catch((error) => this.snackBarService.showErrorMessage(error));

        this.deletePostHub.on('DeletePost', (deletePost: Post) => {
            if (deletePost) {
